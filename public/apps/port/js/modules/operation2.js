...
// ========== 🚀 v1.8: 4 种调度算法配置 ==========
window.ALGO_CONFIG = {
    fifo:       { name: '传统FIFO',     emoji: '🔴', color: '#ff6b6b', rgba: '255,107,107' },
    time:       { name: '⏱️ 时间最优',   emoji: '🔵', color: '#00b4ff', rgba: '0,180,255'   },
    throughput: { name: '📦 吞吐量最大', emoji: '🟢', color: '#00ff88', rgba: '0,255,136'   },
    wait:       { name: '⚖️ 最大等待最短',   emoji: '🟣', color: '#9d4edd', rgba: '157,78,221'  }
};
...
window.simPipelines = {
    fifo: [
        // ✂️ 模式2：去掉 8004/8005/8006/8007 入口段（卸车房不再出现）
        // 6个货场作为虚拟起点，本身不占用管线

        // A/B 货场出货段（由1号/2号转载站对接）
        {id: '8013', name: '8013管线', busy: false, freeAt: 0},   // → 4号转载站
        {id: '8094', name: '8094管线', busy: false, freeAt: 0},   // → 7号转载站
        {id: '8014', name: '8014管线', busy: false, freeAt: 0},   // → 7号转载站
        {id: '8015', name: '8015管线', busy: false, freeAt: 0},   // → 8号转载站
        {id: 'BC1-1', name: 'BC1-1管线', busy: false, freeAt: 0}, // → T1转载站（装船）
        {id: 'BC1-2', name: 'BC1-2管线', busy: false, freeAt: 0}, // → T1转载站（装船）

        // C/D 货场出货段（由7号/3号转载站对接）
        {id: '8017', name: '8017管线', busy: false, freeAt: 0},   // 7号 → 3号
        {id: '8019', name: '8019管线', busy: false, freeAt: 0},   // 3号 → 4号
        {id: '8020', name: '8020管线', busy: false, freeAt: 0},   // 3号 → 4号
        {id: '8101', name: '8101管线', busy: false, freeAt: 0},   // 4号 → 5号转载站
        {id: '8102', name: '8102管线', busy: false, freeAt: 0},   // 4号 → 5号转载站

        // E/F 货场出货段（由8号/9号转载站对接）
        {id: '8109', name: '8109管线', busy: false, freeAt: 0},   // 8号 → 9号
        {id: '8110', name: '8110管线', busy: false, freeAt: 0},   // 9号 → 3号 ✅新增（文档补全）
        ...
        // 仓库 → 6号转载站（仓库出港转装船）
        {id: '8043', name: '8043管线', busy: false, freeAt: 0},   // ✅新增（文档补全）
...
// 🆕 模式2：6个货场定义（虚拟起点，不占管线，对应1/2/3/7/8/9号转载站）
// 🆕 模式2:6个货场定义(虚拟起点,不占管线,对应1/2/3/7/8/9号转载站)
// 每个堆场储存2-3种煤(库存视为无限大),煤种分布保证每个煤种都至少在3个堆场可得
window.YARDS = [
    { id: 'A', name: 'A货场', linkedStations: ['1','2'], coalTypes: ['无烟煤','烟煤','肥煤'] },
    { id: 'B', name: 'B货场', linkedStations: ['1','2'], coalTypes: ['焦煤','气煤','褐煤'] },
    { id: 'C', name: 'C货场', linkedStations: ['7','3'], coalTypes: ['烟煤','肥煤','焦煤'] },
    { id: 'D', name: 'D货场', linkedStations: ['7','3'], coalTypes: ['无烟煤','褐煤','气煤'] },
    { id: 'E', name: 'E货场', linkedStations: ['8','9'], coalTypes: ['肥煤','气煤'] },
    { id: 'F', name: 'F货场', linkedStations: ['8','9'], coalTypes: ['无烟煤','烟煤','焦煤','褐煤'] }
];

// 🆕 模式2:根据煤种找出所有可用堆场
window.findYardsByCoalType = function(coalType) {
    return window.YARDS.filter(y => y.coalTypes.includes(coalType)).map(y => y.id);
};
...
const SHIP_POOL_STORAGE_KEY = 'port_sim_globalShipPool_mode2';  // 🆕 模式2独立key,避免与模式1互相覆盖
...
function saveShipPoolToStorage() {
    try {
        ...
        const clean = window.globalShipPool.map(s => ({
            ...
            originYard: s.originYard,           // 🆕 模式2: 起始货场
            candidateYards: s.candidateYards    // 🆕 模式2: 候选货场清单
        }));
...
window.operationSimState = {
    params: { pipelineFlow: 2000, berthCount: 6, shipCapacity: 2000, parallelCapacity: 8, predictionSteps: 5, shipDensity: 'medium' },
...
function generateShipListTable() {
    ...
    html += '<th style="padding:8px 6px;text-align:center;color:#2ed573;border:1px solid rgba(46,213,115,0.3);">🏭 起始货场</th>';
    ...
    allShips.forEach(ship => {
        ...
        // 🆕 模式2:起始货场列
        const candidStr = (ship.candidateYards || []).join(',');
        const yardTip = candidStr ? '候选: ' + candidStr : '';
        html += '<td style="padding:6px 4px;text-align:center;color:#2ed573;border:1px solid rgba(46,213,115,0.2);font-weight:bold;" title="' + yardTip + '">' + (ship.originYard || '-') + '</td>';
...
function generateDispatchDetailTable(mode) {
    ...
    if (allShips.length === 0) {
        const colspan = isSmartMode ? 13 : 10;
    ...
    allShips.forEach(ship => {
        ...
        // 🆕 模式2:起始货场列
        const yardCandidates = (ship.candidateYards || []).join(',');
        const yardTitle = yardCandidates ? '该煤种候选货场: ' + yardCandidates : '';
        html += '<td style="padding:6px 4px;text-align:center;color:#2ed573;font-weight:bold;" title="' + yardTitle + '">' + (ship.originYard || '-') + '</td>';
...
function generateHTML() {
    ...
    window.ALGO_KEYS.forEach(key => {
        ...
        const desc = key === 'fifo' ? '严格按到达顺序调度'
                   ...
                   : 'HRRN高响应比，最长等待最短（防饥饿）';
        module3 += '<div style="color: #7a9aba; font-size: 11px; margin-top: 4px;">' + desc + '</div></div>';
    });
    module3 += '</div></div>';
    ...
    window.ALGO_KEYS.forEach(key => {
        ...
        // 公共表头 9 列
        const headers = ['船舶编号', '煤炭种类', '🏭起始货场', '货量', '使用管线', '到港时间', '开始装载', '完成时间', '等待(分)', '状态'];
        const aligns = ['left', 'left', 'center', 'center', 'left', 'center', 'center', 'center', 'center', 'center'];
        headers.forEach((h, i) => {
    ...
    explanation += '<span style="color:#00ff88">📦 吞吐量最大</span>：大货量优先 + 同煤种并行，单位时间装的最多<br>';
...
function generateFlowSVG(mode) {
    ...
    function addYard(x, y, label, id) {
        // 🆕 模式2:显示煤种+库存∞;从 window.YARDS 自动读取煤种
        const Y = window.YARDS || [];
        const yardData = Y.find(z => z.id === label);
        const coals = yardData ? yardData.coalTypes : [];
        const coalStr = coals.join('/');
        // 矩形稍微高一些,塞下煤种
        let r = '<g id="' + mode + '-yard-' + id + '">';
        r += '<rect x="' + x + '" y="' + y + '" width="110" height="62" rx="5" fill="rgba(46,213,115,0.18)" stroke="#2ed573" stroke-width="2"/>';
        r += '<text x="' + (x+55) + '" y="' + (y+16) + '" text-anchor="middle" fill="#2ed573" font-size="11" font-weight="bold">' + label + ' 货场</text>';
        r += '<text x="' + (x+55) + '" y="' + (y+32) + '" text-anchor="middle" fill="#fff" font-size="8">' + coalStr + '</text>';
        r += '<text id="' + mode + '-yard-' + id + '-inventory" x="' + (x+55) + '" y="' + (y+52) + '" text-anchor="middle" fill="#ffd93d" font-size="10" font-weight="bold">库存 ∞</text>';
        r += '</g>';
        return r;
    }
    
    // ========== 🆕 模式2 Layer 1 (y=30)：6个货场作为起点 -> 直接对接转载站 ==========
    // 文档：A/B→1/2号  C/D→3/7号  E/F→8/9号
    svg += '<g id="' + mode + '-layer1">';
    svg += '<text x="450" y="14" text-anchor="middle" fill="' + c + '" font-size="11" font-weight="bold">🏭 起点：6个货场（船舶到港直接从货场出货，不再走卸车房）</text>';

    // 6 个货场紧凑摆放在顶部（y=25），等间距 6 列
    // 每列宽 130，从 x=50 开始：A(50) B(180) C(330) D(460) E(610) F(740)
    svg += addYard(50,  25, 'A', 'A');
    svg += addYard(190, 25, 'B', 'B');
    svg += addYard(340, 25, 'C', 'C');
    svg += addYard(490, 25, 'D', 'D');
    svg += addYard(640, 25, 'E', 'E');
    svg += addYard(790, 25, 'F', 'F');

    // 1号/2号转载站（接 A/B 货场）—— 保留供下游连接
    svg += addRect(60, 95, 130, 25, '1号/2号转载站');
    // A→1/2 (短直线)
    svg += addCurveArrow(100, 75, 100, 82, 100, 88, 100, 95, '', c, false);
    // B→1/2
    svg += addCurveArrow(230, 75, 220, 82, 200, 88, 170, 95, '', c, false);

    // 7号转载站（接 C/D 货场）
    svg += addRect(340, 95, 130, 25, '7号转载站');
    // C→7
    svg += addCurveArrow(380, 75, 380, 82, 380, 88, 380, 95, '', c, false);
    // D→7
    svg += addCurveArrow(510, 75, 500, 82, 470, 88, 440, 95, '', c, false);

    // 8号转载站（接 E/F 货场）
    svg += addRect(620, 95, 130, 25, '8号转载站');
    // E→8
    svg += addCurveArrow(660, 75, 660, 82, 660, 88, 660, 95, '', c, false);
    // F→8
    svg += addCurveArrow(790, 75, 780, 82, 750, 88, 720, 95, '', c, false);
...
window.nextStep = function() {
    ...
    ALGO_KEYS.forEach(modeKey => {
        ...
        const tp = st.ships.filter(x => x.processed).reduce((sum, x) => sum + x.cargoSize, 0);
        const sc = st.ships.filter(x => x.processed).length;
        console.log(`📊 ${cfg.emoji} ${cfg.name} - 总船舶: ${st.ships.length}, 已完成: ${sc}, 吞吐量: ${tp.toLocaleString()} 吨`);
    });
    console.log('══════════════════════════════════════════════════');
...
function runSchedulerForMode(modeKey, currentTime) {
    ...
    st.ships.forEach(ship => {
        if (ship.assigned && !ship.processed && currentTime >= ship.endTime) {
            ship.processed = true;
            st.shipsProcessed++;
    ...
    waiting.forEach(ship => {
        ...
        if (modeKey === 'time') {
            ...
            const isStarve = wait >= 4 ? '🔥饥饿' : '';
            ship.decisionNote = '⚖️ 最大等待最短（已等' + wait + '步' + isStarve + '）';
        }
    ...
    if (modeKey === 'throughput') {
        ...
        // ⚖️ 最大等待最短 (模式2重设):
        // 主调: 按到达时间排序（与 FIFO 一致 → 不会有船被无限期拖后）
        // 加护: 等超过阈值的船绝对插队（饥饿逃生阀）
        // 优势: 靠「智能选货场 + 选最快路径」压低最大等待，而不是靠重排序
        const STARVATION_THRESHOLD = 4;  // 超过 4 步（30分钟×4=2小时）未作业→饥饿
        normalShips.sort((a, b) => {
            const aArrived = (a.arrivalStep || 0) <= currentTime ? 0 : 1;
            const bArrived = (b.arrivalStep || 0) <= currentTime ? 0 : 1;
            if (aArrived !== bArrived) return aArrived - bArrived;
            ...
            // 🔥 饥饿逃生: 超阈值的船优先于未超阈值的
            const aStarve = aWait >= STARVATION_THRESHOLD ? 1 : 0;
            const bStarve = bWait >= STARVATION_THRESHOLD ? 1 : 0;
            if (aStarve !== bStarve) return bStarve - aStarve;
            // 同饥饿级内: 等久的优先
...
function updateDisplay() {
    ...
    ALGO_KEYS.forEach(key => {
        ...
        const pipeCount = pipes.length || totalPipelinesCount || 1;
        // 🆕 累计近10步利用率(更稳定)
        if (!window.__utilHistory) window.__utilHistory = { fifo:[], time:[], throughput:[], wait:[] };
        const hist = window.__utilHistory[key];
        hist.push(busy / pipeCount);
        if (hist.length > 10) hist.shift();
        const recentAvg = hist.reduce((a,b)=>a+b, 0) / hist.length;
        const util = Math.min(Math.round(recentAvg * 100), 100);
...
function ensurePipelineState(pipe) {
    if (!pipe.activeShips) pipe.activeShips = [];  // 正在装载的船 [{shipId, coal, endAt, flow}]
    if (typeof pipe.flowUsed !== 'number') pipe.flowUsed = 0;
    return pipe;
}

// 释放管线上已结束的船舶占用，更新流量与煤种
function releasePipelineExpiredShips(pipe, currentTime) {
    ensurePipelineState(pipe);
    if (pipe.activeShips.length === 0) return;
    const remaining = pipe.activeShips.filter(a => a.endAt > currentTime);
    pipe.activeShips = remaining;
    pipe.flowUsed = remaining.reduce((sum, a) => sum + a.flow, 0);
    if (remaining.length === 0) {
        pipe.busy = false;
        // 保留 lastCoalType 用于下次清舱判断
    } else {
        pipe.busy = true;
        pipe.freeAt = Math.max.apply(null, remaining.map(a => a.endAt));
    }
}
...
function canPipelineAcceptShip(pipe, ship, currentTime, pipelineFlow) {
    ensurePipelineState(pipe);
    const shipFlow = computeShipFlowDemand(ship);

    // case 1: 管线完全空闲（没有活跃船舶）
    if (pipe.activeShips.length === 0) {
        // 需要判断清舱时间：上一次装的煤种和本次不同 → 1步清舱
        let cleanSteps = 0;
    ...
    // case 2: 管线正在装载 - 必须同煤种 + 流量够 才能并行加入（核心新功能！）
    const currentCoal = pipe.activeShips[0].coal;
    if (currentCoal === ship.coalType && pipe.flowUsed + shipFlow <= pipelineFlow) {
        return { ok: true, mode: 'parallel', cleanSteps: 0, waitUntil: currentTime };
    }

    // case 3: 管线被占且无法并行 → 等待最早一艘完成
    const earliestEnd = Math.min.apply(null, pipe.activeShips.map(a => a.endAt));
...
function occupyPath(mode, path, ship, currentTime, loadSteps, cleanSteps) {
    ...
    path.forEach(pipeId => {
        ...
        pipe.flowUsed = pipe.activeShips.reduce((s, a) => s + a.flow, 0);
        pipe.busy = true;
...
// 获取所有可用路径（按默认优先级排序）
function getAllAvailablePaths(destination, cargoType, originYard) {
    // 🆕 模式2：路径以"货场"为起点（去掉卸车房+8004-8007入口段）
    // originYard: 'A'/'B'/'C'/'D'/'E'/'F' （为兼容老代码，缺省随机）
    if (!originYard) {
        const yards = ['A','B','C','D','E','F'];
        originYard = yards[Math.floor(Math.random()*yards.length)];
    }

    const allPipelines = ['8013','8094','8014','8015','BC1-1','BC1-2',
                          '8017','8019','8020','8101','8102',
                          '8109','8110','8021','8022','8105','8106','8043','BC3-1','BC3-2'];
    const availablePipeIds = getAvailablePipelines(allPipelines);
    const paths = [];

    // ===== 终点：仓库（汽车装运）=====
    if (destination === 'warehouse') {
        if (originYard === 'A' || originYard === 'B') {
            // A/B → 1/2号 → 4号 → 仓库
            tryAdd(['8013','8021'], 1, 'A/B→4号→仓库(直通)');
            tryAdd(['8013','8022'], 2, 'A/B→4号→仓库(备用)');
            // A/B → 7号 → 3号 → 4号 → 仓库
            tryAdd(['8094','8017','8019','8021'], 3, 'A/B→7号→3号→4号→仓库');
            tryAdd(['8014','8017','8020','8022'], 4, 'A/B→7号→3号→4号→仓库(备用)');
        } else if (originYard === 'C' || originYard === 'D') {
            // C/D → 7号 → 3号 → 4号 → 仓库
            tryAdd(['8017','8019','8021'], 1, 'C/D→7号→3号→4号→仓库');
            tryAdd(['8017','8020','8022'], 2, 'C/D→7号→3号→4号→仓库(备用)');
            // C/D → 3号(直对接) → 4号 → 仓库
            tryAdd(['8019','8021'], 3, 'C/D→3号→4号→仓库');
            tryAdd(['8020','8022'], 4, 'C/D→3号→4号→仓库');
        } else if (originYard === 'E' || originYard === 'F') {
            // E/F → 8号 → 9号 → 3号 → 4号 → 仓库
            tryAdd(['8109','8110','8019','8021'], 1, 'E/F→8号→9号→3号→4号→仓库');
            tryAdd(['8109','8110','8020','8022'], 2, 'E/F→8号→9号→3号→4号→仓库');
        }
    }
    // ===== 终点：装船(T3) =====
    else {
        if (originYard === 'A' || originYard === 'B') {
            // A/B → 1/2号 → T1 → T3（最短装船路径）
            tryAdd(['BC1-1','BC3-1'], 1, 'A/B→T1→T3(快速装船1)');
            tryAdd(['BC1-2','BC3-2'], 2, 'A/B→T1→T3(快速装船2)');
            // A/B → 4号 → 5号 → 6号 → T3
            tryAdd(['8013','8101','8105','BC3-1'], 3, 'A/B→4号→5号→6号→T3');
            tryAdd(['8013','8102','8106','BC3-2'], 4, 'A/B→4号→5号→6号→T3');
            // A/B → 1/2号 → 仓库 → 6号 → T3
            tryAdd(['8013','8021','8043','BC3-1'], 5, 'A/B→4号→仓库→6号→T3');
        } else if (originYard === 'C' || originYard === 'D') {
            // C/D → 3号 → 4号 → 5号 → 6号 → T3
            tryAdd(['8019','8101','8105','BC3-1'], 1, 'C/D→3号→4号→5号→6号→T3');
            tryAdd(['8020','8102','8106','BC3-2'], 2, 'C/D→3号→4号→5号→6号→T3');
            // C/D → 7号 → 3号 → 4号 → 5号 → 6号 → T3
            tryAdd(['8017','8019','8101','8105','BC3-1'], 3, 'C/D→7号→...→T3');
            tryAdd(['8017','8020','8102','8106','BC3-2'], 4, 'C/D→7号→...→T3');
        } else if (originYard === 'E' || originYard === 'F') {
            // E/F → 8号 → 9号 → 3号 → 4号 → 5号 → 6号 → T3
            tryAdd(['8109','8110','8019','8101','8105','BC3-1'], 1, 'E/F→8号→9号→...→T3');
            tryAdd(['8109','8110','8020','8102','8106','BC3-2'], 2, 'E/F→8号→9号→...→T3');
        }
    }

    function tryAdd(path, priority, desc) {
        if (path.every(p => availablePipeIds.includes(p))) {
            paths.push({ path, priority, desc, originYard });
        }
    }
    ...
    if (paths.length === 0) {
        if (destination === 'warehouse') {
            paths.push({ path: ['8013','8021'], priority: 1, desc: '主路径(兜底)', originYard });
        } else {
            paths.push({ path: ['BC1-1','BC3-1'], priority: 1, desc: '主路径(兜底)', originYard });
        }
    }
    return paths;
}
...
function fifoScheduler(ship, currentTime) {
    ...
    const pipelineFlow = (window.operationSimState && window.operationSimState.params && window.operationSimState.params.pipelineFlow) || 2000;

    // 🆕 模式2: FIFO调度时选起始货场（轮转到负载最轻的场，避免都压 A/B）
    if (!ship.originYard && ship.candidateYards && ship.candidateYards.length > 0) {
        const stateFifo = (window.operationSimState.states && window.operationSimState.states.fifo) || null;
        const yardLoad = { A:0, B:0, C:0, D:0, E:0, F:0 };
        if (stateFifo) {
            stateFifo.ships.forEach(sh => {
                if (sh.originYard && yardLoad[sh.originYard] !== undefined) {
                    yardLoad[sh.originYard]++;
                }
            });
        }
        // 按负载升序 + 原始候选顺序选
        const sortedCands = [...ship.candidateYards].sort((a, b) => {
            const la = yardLoad[a] || 0, lb = yardLoad[b] || 0;
            if (la !== lb) return la - lb;
            return ship.candidateYards.indexOf(a) - ship.candidateYards.indexOf(b);
        });
        ship.originYard = sortedCands[0];
        ship.yardSelectStrategy = 'FIFO-轮转载载轻场';
    }

    const allPaths = getAllAvailablePaths(ship.destination, ship.cargoType, ship.originYard);
...
function smartScheduler(ship, currentTime, modeKey) {
    ...
    // 🆕 模式2: 调度时按算法策略选起始货场（含负载均衡，避免都压到 A/B）
    if (!ship.originYard && ship.candidateYards && ship.candidateYards.length > 0) {
        const cands = ship.candidateYards;
        let bestYard = cands[0];
        let bestScore = Infinity;

        // 统计该 mode 下每个货场被分配过多少艙船（负载）
        const stateForMode = (window.operationSimState.states && window.operationSimState.states[modeKey]) || null;
        const yardLoad = { A:0, B:0, C:0, D:0, E:0, F:0 };
        if (stateForMode) {
            stateForMode.ships.forEach(sh => {
                if (sh.originYard && yardLoad[sh.originYard] !== undefined) {
                    yardLoad[sh.originYard]++;
                }
            });
        ...
        // 遍历每个候选货场，按该货场生成路径，评分后选最佳
        cands.forEach(yardId => {
            const trialPaths = getAllAvailablePaths(ship.destination, ship.cargoType, yardId);
            if (!trialPaths || trialPaths.length === 0) return;
            const bestPath = trialPaths[0];
            const pathLen = bestPath.path.length;

            // 该路径上的忙碌指标
            let busyCount = 0;
            let waitUntil = 0;
            for (const pipeId of bestPath.path) {
                const pipe = findPipeline(modeKey, pipeId);
                if (pipe && pipe.busy && currentTime < pipe.freeAt) {
                    busyCount++;
                    waitUntil = Math.max(waitUntil, pipe.freeAt - currentTime);
                }
            }

            // 🆕 负载惩罚：被选过越多的货场加分（防止都压一个）
            const loadPenalty = (yardLoad[yardId] || 0) * 8;

            let score;
            if (modeKey === 'time') {
                // ⏱️ SJF: 路径短 + 等待少 + 负载轻
                score = pathLen * 8 + waitUntil * 15 + loadPenalty;
            } else if (modeKey === 'throughput') {
                // 📦 吞吐量: 鼓励同路径复用，但也要均衡
                score = pathLen * 5 - busyCount * 3 + loadPenalty * 0.6;
            } else if (modeKey === 'wait') {
                // ⚖️ HRRN: 最快开装 + 负载均衡很重要（避免某场出口拥堵）
                score = waitUntil * 30 + pathLen * 5 + loadPenalty * 1.5;
            } else {
                // FIFO / 默认: 优先轮转到载载最轻的场（公平）
                score = (yardLoad[yardId] || 0) * 100 + cands.indexOf(yardId);
            }
        ...
        ship.originYard = bestYard;
        ship.yardSelectStrategy = modeKey === 'time' ? 'SJF-最短路径+负载轻'
                               : modeKey === 'throughput' ? 'Throughput-同路径复用'
                               : modeKey === 'wait' ? 'HRRN-最快开装+分散'
                               : '默认-首位候选';
    }
...
function generateShip(shipId) {
    ...
    // 🆕 模式2:仅记录该煤种可用的候选堆场,originYard 留空,由调度器在执行时决定
    const candidateYards = (typeof window.findYardsByCoalType === 'function')
        ? window.findYardsByCoalType(coalType)
        : ['A','B','C','D','E','F'];
    const safeCandidates = candidateYards.length > 0 ? candidateYards : ['A','B','C','D','E','F'];
    ...
    return {
        ...
        originYard: null,                         // 🆕 调度时才确定(原始清单为空)
        candidateYards: safeCandidates,           // 🆕 该煤种可用的所有候选货场
        cargoSize: cargoSize,
        coalType: coalType,
...
function geneticScheduler(currentShip, futureShips, currentTime) {
    const mode = 'smart';
    const allPaths = getAllAvailablePaths(currentShip.destination, currentShip.cargoType, currentShip.originYard);
